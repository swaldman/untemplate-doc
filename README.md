...
* <a href="#untemplate-documentation">Untemplate documentation</a>
  ...
  * <a href="#feature-creep">Feature Creep</a>
    ...
    * <a href="#side-scala-files">Side Scala Files</a>
  * <a href="#mill-plugin">Mill Plugin</a>
...
> :bulb: **Mnemonic** <br/>
> For every construct, whatever an "arrow" &mdash; `<` or `>`&mdash points at is a text region. Whatever is adjacent to a parenthesis &mdash; `(` or `)` or `()` &mdash; is code.
...
```scala
// DO NOT HAND EDIT -- Autogenerated from 'ceci-nest-pas.md.untemplate' at 2025-08-16T21:14:28.333466Z
...
val Untemplate_ceci_nest_pas_md = new untemplate.Untemplate[immutable.Map[String,Any],Nothing]:
  ...
  val UntemplateLastModified                : Option[Long] = Some(1673166580000L)
...
```markdown
# Ceci n'est pas... 0.6387636934412373
...
> **Note** <br/>
> This `README.md` is [generated by](untemplatedoc/untemplate/untemplatedoc/readme/frame-main.md.untemplate) an untemplate! [[current subsection](untemplatedoc/untemplate/untemplatedoc/readme/somesimpleuntemplates/content-embeddable-expressions.md.untemplate)]
...
`.untemplate`. The top-level file you are reading is [`frame-main.md.untemplate`](untemplatedoc/untemplate/untemplatedoc/readme/frame-main.md.untemplate), and [generates a
function](example/scalagen/untemplatedoc/readme/frame-main-md-untemplate.scala) like...
...
_I was published on Tue, 2 Sep 2025 21:17:18 -0400._
...
For example, [this untemplate's](untemplatedoc/untemplate/untemplatedoc/readme/featurecreep/content-attributes.md.untemplate) header includes
...
Find where that appears in the [generated scala](example/scalagen/untemplatedoc/readme/featurecreep/content-attributes-md-untemplate.scala).
...
```scala
// DO NOT HAND EDIT -- Autogenerated at 2025-08-17T02:32:17.840390Z
package untemplatedoc
...
val Untemplates = scala.collection.immutable.SortedMap[String,untemplate.Untemplate[Nothing,Any]](
  ...
  "untemplatedoc.readme.content_mill_plugin_md" -> untemplatedoc.readme.Untemplate_content_mill_plugin_md,
)
...
* You can replace the default location for untemplate source code via
  ...
  below two directories, the untemplate from the later directory in the list overrides and shadows
...
OK. First let's get a `mill` project set-up:
...
Now let's setup a minimalist `build.mill` file. However you like to create a text file,
create `build.mill` in the current directory (not in `src` or `untemplate`), with the following contents:
...
Great. Let's work in a `mypkg` package. Let's create a directory under `untemplate`.

```plaintext
% mkdir untemplate/mypkg
```

OK! Let's drop our first untemplate into there. Inside `untemplate/mypkg` create a file called `hello.txt.untemplate` with the following contents:
...
Let's write a simple program, and put it `src/hello.scala`:
...
(Note how, in order to call the function, we took the untemplate file name, but dropped the `.untemplate` suffix, and the '.' in the name became an underscore. '-' characters would also become underscore.)
...
1. Our untemplate actually accepts a `String` name for its argument.
   ...
   (You can hardcode the name, or let it be a command-line argument.)
...
2. Most real applications will want an index of untemplates. Add to your
   ...
   (For now there's just one!) The index will be Scala `Map` called `mypkg.Untemplates` (the name we gave it!),
   with the fully qualified names
   of the untemplates as keys and the untemplate functions as values.

   **Hint:** _The `toString` method of the untemplate functions include their type parameterizations,
   but the compiler only knows them as `untemplate.Untemplate[Nothing,Any]`. Try_
   ```scala
   mypkg.Untemplates.head(1).asInstanceOf[untemplate.Untemplate[String,Nothing]]("my friend")
   ```
   _You'll need to use `asInstanceOf` on untemplates retrieved via the index, and you'll generally
   want to deploy conventions so you know their argument and return metadata types from
   their package or name._
...
and [ScalaXB](https://scalaxb.org/)'s [plugin](https://github.com/vic/mill-scalaxb),
